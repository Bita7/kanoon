<html>
<head>
    <title>سایت آموزش آنلاین</title>
    <link rel="stylesheet" href="/css/style.css" >
    <script src="/js/jquery.js"></script>
    <script src="/js/script.js"></script>

</head>
<body DIR="rtl">
<div id="topmenu">
    <div id="right-topmenu">
        <ul>
            <li><a href="">صفحه شخصی شما</a></li>
            <li><a href="">برنامه نویسی</a></li>
            <li><a href="">نفرات اول</a></li>
            <li><a href="">اپلیکیشن موبایل</a></li>
            <li><a href="">EN</a></li>
        </ul>
    </div>
    <div id="left-topmenu">دوشنبه 13دی 1401</div>
</div>
<div id="header">
    <div id="header_part1">
        <form>
            <input type="text" placeholder="جست و جو کنید">
            <input type="submit" value="search">

        </form>
    </div>
</div>
<div id="loginform">
    <div id="loginformtitle">صفحه شخصی اعضا</div>
    <div><label>نام کاربری:</label><input type="text" name="username" class="input"></div>

    <div><label>پسورد:</label><input type="password" name="password" class="input"></div>
    <div><input type="submit" name="btn" class="loginbtn" value="ورود کاربر"></div>
    <div class="msg">رمز عبور خود را فر اموش کرده اید؟</div>
</div>
</body>


</html>
